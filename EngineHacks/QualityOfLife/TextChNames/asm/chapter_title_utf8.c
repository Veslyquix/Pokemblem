...
const u8 sChapterTitleUtf8ColorMap[4] = {
    0, // transparent
    7, // unused ??
    7, // black
    1  // white
};
int ChapterTitleDrawUtf8Glyph(const struct Utf8Glyph *glyph, void *destination,
                              int x) {
  ...
  for (y = 0; y < 16; ++y) {
    ...
    for (pixelX = 0; pixelX < 16; ++pixelX) {
      int pixel = (bitmap[sourceRow + (pixelX >> 2)] >> ((pixelX & 3) * 2)) & 3;
      int destinationX;
      int byteOffset;
      int shift;
      volatile u32 *row;
      ...
      byteOffset =
          ((y >> 3) * 0x400) + ((destinationX >> 3) * 0x20) + ((y & 7) * 4);
      shift = (destinationX & 7) * 4;
      row = (volatile u32 *)(tiles + byteOffset);
      value = *row;
      value &= ~(0xFu << shift);
      value |= (u32)sChapterTitleUtf8ColorMap[pixel] << shift;
      *row = value;
    }
  }
