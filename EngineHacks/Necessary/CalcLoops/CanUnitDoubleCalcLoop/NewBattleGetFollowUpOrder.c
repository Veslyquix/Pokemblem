...
#include <stdlib.h>
// see
// https://github.com/FireEmblemUniverse/fireemblem8u/blob/f3fc2db675198eba47b075e3a94a6284f576df90/src/bmbattle.c#L825

extern int DoublingThresholdLink;

extern s8 BattleGenerateRoundHits(struct BattleUnit *attacker,
                                  struct BattleUnit *defender);
extern void BattleGetBattleUnitOrder(struct BattleUnit **outAttacker,
                                     struct BattleUnit **outDefender);
extern void ClearBattleHits(void);
extern void BattleForecastHitCountUpdate(struct BattleUnit *battleUnit,
                                         u8 *hitsCounter, int *usesCounter);
extern int IsUnitEffectiveAgainst(struct BattleUnit *attacker,
                                  struct BattleUnit *defender);
extern s8 BattleGetFollowUpOrder(struct BattleUnit **outAttacker,
                                 struct BattleUnit **outDefender);

int DoesUnitImmediatelyFollowUp(struct BattleUnit *bunitA,
                                struct BattleUnit *bunitB);
...
s8 NewBattleGetFollowUpOrder(struct BattleUnit **outAttacker,
                             struct BattleUnit **outDefender);
...
int PassionsFlow(struct BattleUnit *bunitA, struct BattleUnit *bunitB) {
  if (SkillTester(&bunitA->unit, PassionsFlowID_Link)) {
    if (bunitA == &gBattleActor) {
      ...
      // this function returns true if any bonuses are found and also puts the
      // bonuses into the provided ram
...
int QuickRiposte(struct BattleUnit *bunitA, struct BattleUnit *bunitB) {
  if (gBattleStats.config &
      (BATTLE_CONFIG_REAL |
       BATTLE_CONFIG_SIMULATE)) { // Quick Riposte as asm did this for some
                                  // reason:
    //@pre-battle data pointer, gonna check if a target has been selected or the
    // fight has started (0x02 if targeting someone, 0x01 if battle started)
...
int RecklessFighter(struct BattleUnit *bunitA, struct BattleUnit *bunitB) {
  if ((SkillTester(&bunitA->unit, RecklessFighterID_Link) &&
       bunitA->hpInitial < bunitA->unit.maxHP) ||
      (SkillTester(&bunitB->unit, RecklessFighterID_Link) &&
       bunitB->hpInitial < bunitB->unit.maxHP)) {
    return ForceDouble;
  }
  return NoChange;
}
...
int DoesUnitImmediatelyFollowUp(struct BattleUnit *bunitA,
  ...
  int hasDesperation = (SkillTester(&bunitA->unit, DesperationID_Link) &&
                        (bunitA->hpInitial < (bunitA->unit.maxHP / 2)));
  int hasAssassinate =
      (SkillTester(&bunitA->unit, AssassinateID_Link) && (dist == 1) &&
       (bunitA == &gBattleActor)); // assassinate only works while attacking
...
void NewBattleUnwind(void) {
  ...
  // which is kind of neat because it implies scrapped plans for supporting some
  // accost kind of thing
  ...
  do {
    ...
    int hasPridefulWarrior =
        (SkillTester(&attacker->unit, PridefulWarriorID_Link));
    ...
    if (!BattleGenerateRoundHits(attacker,
                                 defender)) { // attacker hits defender
      ...
      if (DoesUnitImmediatelyFollowUp(attacker, defender)) {
        ...
        if (!desperationEnds) {
          ...
          int countered = BattleGenerateRoundHits(
              &gBattleTarget,
              &gBattleActor); // defender (potentially) counter attacks
          if (!countered) {
        ...
        gBattleHitIterator->attributes |= BATTLE_HIT_ATTR_RETALIATE;
        int countered = BattleGenerateRoundHits(
            defender, attacker); // defender (potentially) counter attacks
        if (!countered) {
  ...
  // asm("mov r11, r0"); // for some reason it sets r11 to 0. r11 is not used by
  // this function.
  //  "Barricade uses r11 for various things through combat. This unsets it
  //  afterward."
}
...
int CanUnitDouble(struct BattleUnit *bunitA, struct BattleUnit *bunitB) {
  ...
  for (int i = 0;; i++) {
    struct UnitDoubleCalcLoop_Struct *doubleCalcLoop =
        &CanUnitDoubleCalcLoop[i];
...
void NewInitBattleForecastBattleStats(struct BattleForecastProc *proc) {
  ...
  if ((gBattleActor.weapon != 0) || (gBattleActor.weaponBroke)) {
    ...
    if (IsUnitEffectiveAgainst((struct BattleUnit *)&gBattleActor.unit,
                               (struct BattleUnit *)&gBattleTarget.unit) != 0) {
      proc->isEffectiveA = 1;
    }

    if (IsItemEffectiveAgainst(gBattleActor.weaponBefore,
                               &gBattleTarget.unit) != 0) {
      proc->isEffectiveA = 1;
    }

    if ((gBattleActor.wTriangleHitBonus > 0) &&
        (gBattleActor.weaponAttributes & IA_REVERTTRIANGLE) != 0) {
      proc->isEffectiveA = 1;
    }
  }
  ...
  if ((gBattleTarget.weapon != 0) || (gBattleTarget.weaponBroke)) {
    BattleForecastHitCountUpdate(&gBattleTarget, (u8 *)&proc->hitCountB,
                                 &usesB);
    ...
    if (IsUnitEffectiveAgainst((struct BattleUnit *)&gBattleTarget.unit,
                               (struct BattleUnit *)&gBattleActor.unit) != 0) {
      proc->isEffectiveB = 1;
    }

    if (IsItemEffectiveAgainst(gBattleTarget.weaponBefore,
                               &gBattleActor.unit) != 0) {
      proc->isEffectiveB = 1;
    }

    if ((gBattleTarget.wTriangleHitBonus > 0) &&
        (gBattleTarget.weaponAttributes & IA_REVERTTRIANGLE) != 0) {
      proc->isEffectiveB = 1;
    }
  }
