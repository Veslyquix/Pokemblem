...
// 	{
// 		if((ptx >= MAP_DIMENSIONS)||(pty >=
// MAP_DIMENSIONS)||(ptx<0)||(pty<0)) return SEA_COLOUR; 		u16 clr2
// = colourMap_sunset[(pty<<MAP_DIMENSIONS_LOG2)+ptx]; 		u16 clr1 =
// colourMap[(pty<<MAP_DIMENSIONS_LOG2)+ptx]; 		return
// iwram_clr_blend_asm(clr1, clr2, sunsetVal);
// 	};
// 	if (sunsetVal){
// 		if((ptx >= MAP_DIMENSIONS)||(pty >=
// MAP_DIMENSIONS)||(ptx<0)||(pty<0)) return SEA_COLOUR_SUNSET;
// return colourMap_sunset[(pty<<MAP_DIMENSIONS_LOG2)+ptx];
// 	}
// 	else {
// 		if((ptx >= MAP_DIMENSIONS)||(pty >=
// MAP_DIMENSIONS)||(ptx<0)||(pty<0)) return SEA_COLOUR; 		return
// colourMap[(pty<<MAP_DIMENSIONS_LOG2)+ptx];
// 	};
// };

// static inline u8 getPtHeight(int ptx, int pty){
// 	if((ptx >= MAP_DIMENSIONS)||(pty >= MAP_DIMENSIONS)||(ptx<0)||(pty<0))
// return 0; 	return heightMap[(pty<<MAP_DIMENSIONS_LOG2)+ptx];
// };
...
static inline void UpdateSprites(SoarProc *CurrentProc) {
  ...
  if (animClock < 0x10)
    ObjInsertSafe(8, 0x68, 0x58, (void *)&gObj_32x32,
                  OAM_ATTR2(PKBaseTID, 2, 0xC)); // player frames
  else if (animClock < 0x20)
    ObjInsertSafe(8, 0x68, 0x58, (void *)&gObj_32x32,
                  OAM_ATTR2(PKBaseTID + 0x10, 2, 0xC));
  else if (animClock < 0x30)
    ObjInsertSafe(8, 0x68, 0x58, (void *)&gObj_32x32,
                  OAM_ATTR2(PKBaseTID + 0x20, 2, 0xC));
  else
    ObjInsertSafe(8, 0x68, 0x58, (void *)&gObj_32x32,
                  OAM_ATTR2(PKBaseTID + 0x30, 2, 0xC));

  if (CurrentProc->ShowMap)
    ObjInsertSafe(8, 176, 0, (void *)&gObj_64x64,
                  OAM_ATTR2(MinimapBaseTID - 1, 2, 0x2)); // draw minimap

  if (CurrentProc->ShowFPS)
  ...
  if (loc > 0) {
    ...
    ObjInsertSafe(8, 0x30, 0x10, (void *)&gObj_32x8,
                  (OAM_ATTR2(LocationBaseTID + ((loc - 1) << 3) + 4, 2,
                             0xe))); // draw in the top corner if we're there
  };
};

// static inline Point getPLeft(int camera_x, int camera_y, int angle, int
// zdist){
// 	// theory: x = table of all possible zdists and angles = 0x10 * 0x200 =
// 0x2000 entries. (signed halfwords?)
// 	// pleft.x = pleftmatrix[angle][zdist]
// 	// pleft.y = pleftmatrix[(0x10-angle)&0xF][zdist] //mirrored about the
// ns axis

// 	Point pleft = {camera_x + pleftmatrix[angle][zdist], camera_y +
// pleftmatrix[(-angle)&0xF][zdist]}; 	return pleft;
// };
...
// 			{
// 				u8 height_on_screen =
// getScrHeight(offsetPoint.x, offsetPoint.y, altitude, zdist);
...
// 						{
// 							clr =
// getPointColour(offsetPoint.x, offsetPoint.y, sunsetVal); //if not in shadow
// 						    if (zdist > (FOG_DISTANCE))
// 						    {
// 						    	// if ((clr ==
// SEA_COLOUR_SUNSET) & (angle>>2 == 3)) fogclr = 0b000000001111111;
...
// ylen, clr, CurrentProc->vid_page); yBuffer[i] = height_on_screen;
// 					}
// 					//cel shading bit
// 					else if ((-ylen)>CEL_SHADE_THRESHOLD) {
// 						*(u16*)((CurrentProc->vid_page)
// + (i<<5) + (i<<7) + (yBuffer[i] - 1)) = 0;
// 					};
// 				};
// 			};
// 		};
...
// static inline void DrawVerticalLine(int xcoord, int ystart, int ylen, u16
// color, u16* vid_page){
// 	// remove checks, assume it's good data
// 	// if ((ylen<0)||(ystart>MODE5_WIDTH)) return; //don't bother drawing
// negatives
// 	// if ((ystart + ylen) > MODE5_WIDTH) ylen = MODE5_WIDTH - ystart;
// //never draw higher than screen 	int offset = (xcoord<<5) +
// (xcoord<<7)+(ystart);  //shifting to replace multiplication by MODE5_WIDTH
