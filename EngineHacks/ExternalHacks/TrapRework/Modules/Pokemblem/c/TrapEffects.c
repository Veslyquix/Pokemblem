...
extern struct Trap *AddTrapExtFix(int x, int y, int type, int ext1, int ext2,
                                  int ext3, int ext4,
                                  int ext5); //! FE8U = (0x0802E2E0+1)
extern const void *GiveCoinsEvent;
extern int CoinsTrapID_Link;

void AlwaysInitTrap(struct EventTrapData *eTrap) {
  AddTrapExtFix(eTrap->data[0], eTrap->data[1], eTrap->type, eTrap->data[2],
                eTrap->data[3], eTrap->data[4], 0, 0);
}
...
struct Trap *NewGetAdjacentTrap(struct Unit *unit, int trapID_low,
                                int trapID_high) {
  ...
  if (FreeMoveRam->use_dir && GetFreeMovementState()) {
    ...
    if (dir == LVFACELEFT)
      x--;
    else if (dir == LVFACERIGHT)
      x++;
    else if (dir == LVFACEDOWN)
      y++;
    else if (dir == LVFACEUP)
      y--;
    trap = GetTrapAt(x, y);
...
int NewObtainCoinsUsability(int trapID) {
  struct CoinsTrap *trap =
      (struct CoinsTrap *)NewGetAdjacentTrapID(gActiveUnit, CoinsTrapID_Link);
  if (trap) {
...
int TrapEffectCleanup(void) {
  ...
  return returnValue; //  play beep sound & end menu on next frame & clear menu
                      //  graphics
}
...
int DisplayTextUsability0x54(void) {
  ...
  if (trap) {
    ...
    // (HelpMsgFlagOffset_Link)))) { // we want it to always be readable
...
int DisplayTextEffect0x50(void) {
  ...
  // above line in case you want to make a version that does set the event id
  // later
...
int DisplayTextEffect(struct Trap *trap) {
  if (trap) {
    // SetNewFlag_No_sC(trap->data[0]); TutorialTraps have a different ram
    // offset!
...
int NewCutBushUsability() {
  if (CheckEventId(ObtainedCutFlag_Link)) {
    struct ObstacleTrap *trap = (struct ObstacleTrap *)NewGetAdjacentTrapID(
        gActiveUnit, CutBushTrapID_Link);
    if (trap) {
      if (!(gActiveUnit->state & US_CANTOING)) {
        return true;
      }
...
int NewRockSmashUsability() {
  if (CheckEventId(RockSmashFlag_Link)) {
    struct ObstacleTrap *trap = (struct ObstacleTrap *)NewGetAdjacentTrapID(
        gActiveUnit, RockSmashTrapID_Link);
    if (trap) {
      if (!(gActiveUnit->state & US_CANTOING)) {
        return true;
      }
...
int NewStrengthBoulderUsability() {
  if (CheckEventId(StrengthBoulderFlag_Link)) {
    struct ObstacleTrap *trap = (struct ObstacleTrap *)NewGetAdjacentTrapID(
        gActiveUnit, StrengthBoulderTrapID_Link);
    if (trap) {
      if (!(gActiveUnit->state & US_CANTOING)) {
        return true;
      }
