...
// We need to change this FE8-Item Range Fix function to include checking Gaiden
// magic. Slot is the inventory slot to check. -1 is check all slots, and 9 can
// represent just for Gaiden magic. Why don't we let -2 mean only loop through
// items in inventory?
...
// The min/max halfword only really appears to apply to special ranges not even
// including seige tomes? Jesus christ this return scheme aaaaaaaaa. This is a
// way to "trick" the compiler into returning into r1.
long long Return_Range_Bitfield(Unit *unit, int slot,
                                int (*usability)(Unit *unit, int item)) {
  long long current = 0;
  if (slot == -1 || slot == -2) {
    // Loop through all items.
    for (int i = 0; i < 5 && GetEnemyWepBySlot(unit, i); i++) {
      if (usability(unit, GetEnemyWepBySlot(unit, i))) {
        current = IncorporateNewRange(
            current, gGet_Item_Range(unit, GetEnemyWepBySlot(unit, i)));
      }
    }
    return (slot == -1 ? IncorporateNewRange(current, GetUnitRangeMaskForSpells(
                                                          unit, usability))
                       : current);
  } else {
    // Get for this specific slot.
    if (slot != 9) {
      return gGet_Item_Range(unit, GetEnemyWepBySlot(unit, slot));
    } else {
...
long long GetUnitRangeMaskForSpells(Unit *unit,
                                    int (*usability)(Unit *unit, int item)) {
  long long current = 0;
  u8 *spells = SpellsGetter(
      unit, (UsingSpellMenu ? UsingSpellMenu
                            : -1)); // If UsingSpellMenu is nonzero, only get
                                    // Gaiden spells of that type.
...
static long long IncorporateNewRange(long long existing, long long new) {
  ...
  return existingMask | newMask |
         ((newMin < existingMin ? newMin : existingMin) << 40) |
         ((newMax > existingMax ? newMax : existingMax) << 32);
}

int RangeUsabilityCheckStaff(Unit *unit, int item) {
  // On top of being able to cast the spell, only return true if this is also a
  // staff.
...
int GM_doesUnitHaveSpecialRange(
    struct Unit *unit) { // based on GetUnitRangeMaskForSpells - Vesly
  ...
  for (i = 0; spells[i]; i++) {
    ...
    // else
    //{
    // if ( usability(unit,spell) ) { current =
    // IncorporateNewRange(current,gGet_Item_Range(unit,spell)); }
    //}
  }
  if (result > 0x10) {
    return true;
...
int GM_GetUnitRangeBySpellIndex(struct Unit *unit, int index) {
  ...
  if (CanCastSpell(unit, spell))
    return gGet_Item_Range(unit, spell);
  return 0;
}
...
int GM_GetNthSpell(struct Unit *unit, int index) {
  ...
  return spell;
  // if ( CanCastSpell(unit,spell)) return spell; // this is only for after
  // you've moved return 0;
}
