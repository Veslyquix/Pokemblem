...
extern SpellList
    *SpellListTable[0xFF]; // Entirely different from vanilla Spell Association
                           // Table. Just indexed by character ID, points to a
                           // ROM spell list.
extern const MenuDefinition SpellSelectMenuDefs;
extern u8 SpellsBuffer[10]; // 0x202B6D0. Undocced RAM. Just a temporary buffer
                            // it seems.
extern u8 SelectedSpell;    // 0x0203F080.
extern u8 UsingSpellMenu; // 0x0203F082. What type of gaiden magic we're using.
                          // 0 if not.
extern u8 DidSelectSpell; // 0x0203F084. Boolean for whether we've selected
                          // something from the spell menu.
...
extern u16 gCaptureUMErrorText;
...
extern void
SetFaceBlinkControlById(int faceId,
                        int idk2); // 0x088006458. faceId is a guess.
extern void ForceMenuItemPanel(
    MenuProc *menu, Unit *unit, int x,
    int y); // 0x0801E684. idk about the x and y parameters. Just a guess.
extern u32 GetUnitRangeMask(Unit *unit, u8 val); // 0x80171E8.
extern u32 GetWeaponRangeMask(int item);         // 0x080170D4.
extern MenuProc *StartMenu_AndDoSomethingCommands(const MenuDefinition *,
                                                  int xScreen, int xLeft,
                                                  int xRight); // 0x804F64C.
extern void DrawItemMenuCommand(TextHandle *text, u16 item, int canUse,
                                u16 *buffer); // 0x08016848.
extern int AttackUMEffect(MenuProc *proc,
                          MenuCommandProc *commandProc);   // 0x08022B30.
...
static int GaidenMagicUMEffectExt(u8 *spellList, MenuProc *proc,
                                  MenuCommandProc *commandProc);
...
int SpellUsability(const struct MenuCommandDefinition *menuEntry, int index,
                   int idk);
