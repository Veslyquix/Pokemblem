...
#define FOR_EACH_IN_MOVEMENT_RANGE(block)                                                                              \
    ...
    else
    {
        switch (PokemblemGetUnitWeaponReachBits(unit, -1))
        {
            ...
            case REACH_RANGE1:
                FOR_EACH_IN_MOVEMENT_RANGE({ PokemblemMapAddInBoundedRange(ix, iy, 1, 1); })

                break;

            case REACH_RANGE1 | REACH_RANGE2:
                FOR_EACH_IN_MOVEMENT_RANGE({ PokemblemMapAddInBoundedRange(ix, iy, 1, 2); })

                break;

            case REACH_RANGE1 | REACH_RANGE2 | REACH_RANGE3:
                FOR_EACH_IN_MOVEMENT_RANGE({ PokemblemMapAddInBoundedRange(ix, iy, 1, 3); })

                break;

            case REACH_RANGE2:
                FOR_EACH_IN_MOVEMENT_RANGE({ PokemblemMapAddInBoundedRange(ix, iy, 2, 2); })

                break;

            case REACH_RANGE2 | REACH_RANGE3:
                FOR_EACH_IN_MOVEMENT_RANGE({ PokemblemMapAddInBoundedRange(ix, iy, 2, 3); })

                break;

            case REACH_RANGE3:
                FOR_EACH_IN_MOVEMENT_RANGE({ PokemblemMapAddInBoundedRange(ix, iy, 3, 3); })

                break;

            case REACH_RANGE3 | REACH_TO10:
                FOR_EACH_IN_MOVEMENT_RANGE({ PokemblemMapAddInBoundedRange(ix, iy, 3, 10); })
            ...
            case REACH_RANGE1 | REACH_RANGE2 | REACH_RANGE3 | REACH_TO10:
                FOR_EACH_IN_MOVEMENT_RANGE({ PokemblemMapAddInBoundedRange(ix, iy, 1, 10); })

                break;

            case REACH_RANGE1 | REACH_TO10:
                FOR_EACH_IN_MOVEMENT_RANGE({ PokemblemMapAddInBoundedRange(ix, iy, 1, 4); })
...
#ifndef USE_ARM
inline void ApplyMapValueAtRow(int iy, int x, int iRange, int value, int setFog)
{
    int xMin = x - iRange;
...
inline void ThumbPokemblemMapAddInRange(int x, int y, int range, int value)
{
    ...
    for (int iRange = range, iy = y; (iy <= y + range) && (iy < gBmMapSize.y); --iRange, ++iy)
    {
        ApplyMapValueAtRow(iy, x, iRange, value, setFog);
    }

    // Lower half (rows above the center)
    for (int iRange = range - 1, iy = y - 1; (iy >= y - range) && (iy >= 0); --iRange, --iy)
    {
        ApplyMapValueAtRow(iy, x, iRange, value, setFog);
    }
}
