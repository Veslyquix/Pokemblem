...
void const FindFreeTile(struct Unit * unit, int * xOut, int * yOut)
{
    ...
    int activeDeployID = gMapUnit[gActiveUnit->yPos][gActiveUnit->xPos];
    ...
    for (iy = gMapSize.y - 1; iy >= 0; --iy)
    {
        for (ix = gMapSize.x - 1; ix >= 0; --ix)
        {
            ...
            if (gMapMovement[iy][ix] >
                14) // I think high movement costs might cause an overflow issue here? So I'm using 1, 2, 15, 255 cost.
                continue;
            ...
            if (!CanUnitCrossTerrain(
                    unit, gMapTerrain[iy][ix])) // movement costs that are 0x80 or higher won't be accepted
                continue;
    ...
    if (*xOut == (-1))
    {
        ...
        MapMovementFillMovementFromPosition(
            unit->xPos, unit->yPos, GenericMovCost); // void MapFloodExtended(int x, int y, i8 const move_table[]);
        ...
        for (iy = gMapSize.y - 1; iy >= 0; --iy)
        {
            for (ix = gMapSize.x - 1; ix >= 0; --ix)
            {
                ...
                if (gMapMovement[iy][ix] > 14) // I think high movement costs might cause an overflow issue here? So I'm
                                               // using 1, 2, 15, 255 cost.
                    continue;
    ...
    if (gActiveUnit)
    {
        ...
        gMapUnit[gActiveUnit->yPos][gActiveUnit->xPos] = activeDeployID;
    }
}
