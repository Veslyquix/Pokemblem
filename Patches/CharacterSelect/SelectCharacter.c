...
static u8 SelectNoOnBPress(struct MenuProc * menu, struct MenuCommandProc * command);
static u8 SelectCharacterMenuEnd_ReturnFalseOnBPress(struct MenuProc * menu, struct MenuCommandProc * command);
...
extern void Font_ResetAllocation(void);
...
extern void NewLoadBattleSpritesForBranchScreen(struct ProcPromoSel * proc);
...
static const struct MenuCommandDefinition MenuCommands_ConfirmationProc[] = {
    {
        .isAvailable = (void *)MenuCommandAlwaysUsable,
        .onDraw = DrawYes,
        .onEffect = SelectYes,
    },
...
void SelectCharacter_StartMenu(struct Struct_SelectCharacterProc * proc)
{
    ...
    if (proc->unitRam[4] && proc->unitRam[3] && proc->unitRam[2] && proc->unitRam[1] && proc->unitRam[0])
        StartMenu(&MenuDef_SelectCharacter5);
    else if (proc->unitRam[3] && proc->unitRam[2] && proc->unitRam[1] && proc->unitRam[0])
        StartMenu(&MenuDef_SelectCharacter4);
    else if (proc->unitRam[2] && proc->unitRam[1] && proc->unitRam[0])
        StartMenu(&MenuDef_SelectCharacter3);
    else if (proc->unitRam[1] && proc->unitRam[0])
        StartMenu(&MenuDef_SelectCharacter2);
    else if (proc->unitRam[0])
        StartMenu(&MenuDef_SelectCharacter1);
}
...
void CharacterSelectDrawUIBox(Struct_SelectCharacterProc * proc)
{
    ...
    if (cost)
        BgMap_ApplyTsa(&gBG1MapBuffer[11][20], &gCostUIBoxTSA, 0); // 20*8 as X, 0*0 as Y

    // Unit* unit = (proc->unitRam[i]);

    if (proc->unitRam[i]->items[0])
        BgMap_ApplyTsa(&gBG1MapBuffer[13][28], &gItemUIBoxTSA, 0);
    if (proc->unitRam[i]->items[1])
        BgMap_ApplyTsa(&gBG1MapBuffer[13][26], &gItemUIBoxTSA, 0);
    if (proc->unitRam[i]->items[2])
        BgMap_ApplyTsa(&gBG1MapBuffer[15][28], &gItemUIBoxTSA, 0);
    if (proc->unitRam[i]->items[3])
        BgMap_ApplyTsa(&gBG1MapBuffer[15][26], &gItemUIBoxTSA, 0);
...
void StartPlatform(CreatorClassProcStruct * proc)
{
    ...
    NewStartMovingPlatform(proc->platformType, 0x118, gCharacterSelectorPlatformHeight);
}
...
static u8 * UnitGetSkillList(struct Unit * unit)
{
...
void CallSwitchInCharacter(MenuProc * proc)
{
    ...
    Struct_SelectCharacterProc * parent_proc = (Struct_SelectCharacterProc *)ProcFind(ProcInstruction_SelectCharacter);
    parent_proc->menu_proc = (Proc *)proc;
...
void SwitchInCharacter(void) // Whenever you scroll or exit / confirm the character menu
{
    ...
    /*
            ...
            //&= ~(3 << 10); // Clear bits 10 and 11 (priority) such that they are 0 (highest priority) and appear above
       the background
    */
    ...
    for (int i = 0; i < 4; i += 1)
    {
        if (unit->items[i])
        {
            if (iconX > 0x19)
            {
                ...
                // DrawIcon(&gBG0MapBuffer[8][iconX],0x70+i,0x5000); // vanilla uses weapon type icons in item icon IDs
                // starting at 0x70 and indexed by weapon type
    ...
    for ( int i = 0 ; i < 8 ; i++ )
    {
            if ( unit->ranks[i] )
            {
                    if (iconX < 0x12)
                    {
                            ...
                            //DrawIcon(&gBG0MapBuffer[8][iconX],0x70+i,0x5000); // vanilla uses weapon type icons in
    item icon IDs starting at 0x70 and indexed by weapon type
                            // what if they have more than 5 weapon types ?
                            iconX += 3;
                    }
                    else
    ...
    */
    ...
    while (skillList[c])
    {
        ...
        {
            DrawSkillIcon(
                &gBG0MapBuffer[iconY][iconX], skillList[c], TILEREF(0, 0xD)); // Tile ref is basically 0, PaletteIndex
            c++;
            iconX += 3;
        }
        else
    ...
    /// FaceProc* faceProc = StartFace(0, GetUnitPortraitId(unit), 200, 8, 2);
    // faceProc->tileData &= ~(3 << 10); // Clear bits 10 and 11 (priority) such that they are 0 (highest priority) and
    ...
    extern int MakeThumb(int (*inte)(Unit *), Unit * unit);
    ...
    TextHandle GoldSymbolHandle = { .tileIndexOffset = gpCurrentFont->tileNext + tile, .tileWidth = 1 };
    ...
    TextHandle GoldNameHandle = { .tileIndexOffset = gpCurrentFont->tileNext + tile, .tileWidth = 4 };
    ...
    TextHandle char0NameHandle = {
        ...
        .xCursor = 2,
        // GetCharTextWidthAscii(GetStringFromIndex(parent_proc->unitRam[0]->pCharacterData->nameTextId),
        // &char0NameHandle.tileWidth)
    ...
    if (CanWeSelectCharacter(0) == 1)
        Text_SetColorId(&char0NameHandle, TEXT_COLOR_GOLD);

    struct TextHandle char1NameHandle = {
        .tileIndexOffset = gpCurrentFont->tileNext + tile, .tileWidth = 6
        //		.xCursor = 2,
    };
    tile += 6;
    ...
    if (CanWeSelectCharacter(4) == 1)
        Text_SetColorId(&char4NameHandle, TEXT_COLOR_GOLD);

    // Display main menu options based on character ID
    if (parent_proc->unitRam[4] && parent_proc->unitRam[3] && parent_proc->unitRam[2] && parent_proc->unitRam[1] &&
        parent_proc->unitRam[0])
    {
    ...
    TextHandle classNameHandle = { .tileIndexOffset = gpCurrentFont->tileNext + tile, .tileWidth = 6 };
    tile += 6;
    ...
    TextHandle baseHandle = { .tileIndexOffset = gpCurrentFont->tileNext + tile, .tileWidth = 3 };
    tile += 3;
    ...
    if (!classProc)
    {
        ...
        struct CreatorClassProcStruct * newProc =
            (struct CreatorClassProcStruct *)ProcStart(ProcInstruction_CreatorClassProc, (Proc *)parent_proc);
...
static int SelectClass(struct MenuProc * menu, struct MenuCommandProc * command)
{
    if (CanWeSelectCharacter(menu->commandIndex) == 2)
        return ME_NONE; // do nothing
    ...
    // Struct_SelectCharacterProc* proc_variables =
    // (void*)(Struct_SelectCharacterProc*)ProcFind(&ProcInstruction_SelectCharacter[0]);
    //  useful probably
...
static int SelectYes(struct MenuProc * menu, struct MenuCommandProc * command)
{
    ...
    Struct_SelectCharacterProc * selectCharProc =
        (Struct_SelectCharacterProc *)ProcFind(ProcInstruction_SelectCharacter);
...
static int SelectNo(struct MenuProc * menu, struct MenuCommandProc * command)
{
    ...
    // Struct_SelectCharacterProc* confirmation_proc =
    // (Struct_SelectCharacterProc*)ProcFind(&ProcInstruction_SelectCharacter); EndProc(confirmation_proc);
...
static u8 SelectNoOnBPress(struct MenuProc * menu, struct MenuCommandProc * command)
{
    return SelectNo(menu, command);
}

static u8 SelectCharacterMenuEnd_ReturnFalseOnBPress(struct MenuProc * menu, struct MenuCommandProc * command)
{
    SelectCharacterMenuEnd_ReturnFalse();
    return ME_END | ME_PLAY_BEEP;
}
...
static const struct MenuCommandDefinition MenuCommands_CharacterProc1[] = {
    {
        .isAvailable = (void *)CanWeSelectCharacter_0,
        .onEffect = SelectClass,
        .onDraw = DrawSelectCharacterCommands,
        .onSwitchIn = (void *)(void *)CallSwitchInCharacter,
...
static const struct MenuCommandDefinition MenuCommands_CharacterProc2[] = {
    {
        ...
        .onEffect = SelectClass,
        .onDraw = DrawSelectCharacterCommands,
        .onSwitchIn = (void *)CallSwitchInCharacter,

    },

    {
        .isAvailable = (void *)CanWeSelectCharacter_1,
        .onEffect = SelectClass,
        .onDraw = DrawSelectCharacterCommands,
        .onSwitchIn = (void *)CallSwitchInCharacter,
...
static const struct MenuCommandDefinition MenuCommands_CharacterProc3[] = {
    ...
    {
        .isAvailable = (void *)CanWeSelectCharacter_2,
        .onEffect = SelectClass,
        .onDraw = DrawSelectCharacterCommands,
        .onSwitchIn = (void *)CallSwitchInCharacter,
...
static const struct MenuCommandDefinition MenuCommands_CharacterProc4[] = {
    ...
    {
        .isAvailable = (void *)CanWeSelectCharacter_3,
        .onEffect = SelectClass,
        .onDraw = DrawSelectCharacterCommands,
        .onSwitchIn = (void *)CallSwitchInCharacter,
...
static const struct MenuCommandDefinition MenuCommands_CharacterProc5[] = {
    ...
    {
        .isAvailable = (void *)CanWeSelectCharacter_4,
        .onEffect = SelectClass,
        .onDraw = DrawSelectCharacterCommands,
        .onSwitchIn = (void *)CallSwitchInCharacter,
...
static const struct MenuDefinition MenuDef_SelectCharacter1 = {
    ...
    .onInit = 0,
    .onBPress = SelectCharacterMenuEnd_ReturnFalseOnBPress,
    .onRPress = 0,
    .onHelpBox = 0,
};
static const struct MenuDefinition MenuDef_SelectCharacter2 = {
