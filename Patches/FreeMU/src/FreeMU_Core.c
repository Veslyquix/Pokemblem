...
void pFMU_MainLoop(struct FMUProc * proc)
{
    ...
    if (ProcFind(&gProc_Menu) || ProcFind(&gProc_Config1) || ProcFind(&gProc_ChapterStatusScreen) ||
        ProcFind(&gProc_Shop) || ProcFind(&gProc_TargetSelection) || ProcFind(&gProc_TradeMenu))
    {
        exit_early = true;
    }
    ...
    if (eventProc)
    { // wait for events that exist for 3+ frames
        if (eventProc->pEventIdk == proc->pEventIdk)
        { // if the same event as last time, increment time
    ...
    */
    ...
    if (proc->updateSMS)
    { // when units go through each other, this is needed
      // afterwards
        RefreshUnitsOnBmMap();
        ...
        if (!(proc->yield))
        {
            if (!(proc->yield_move))
            {
                if (iKeyCur & 0xF0)
                {
                    ...
                    if (proc->smsFacing == facing)
                    { // if we are turning directions,
                      // don't run events again
                        if (pFMU_RunLocBasedAsmcAuto(proc) == yield)
                        {
                            ...
                            if (eventProc)
                            {
                                proc->yield = true;
                                proc->countdown = 3;
                                if (FreeMoveRam->pause || FreeMoveRam->silent || eventProc->evStallTimer ||
                                    eventProc->pUnitLoadData || eventProc->activeTextType)
                                {
                                    proc->countdown = 2;
...
u16 FMU_FilterMovementInput(struct FMUProc * proc, u16 iKeyCur)
{
    ...
    if (iKeyUse)
    { // most recently pressed key, even if multiple are held down
        iKeyCur = iKeyUse;
    }
...
{ // so we game over with only our Protag alive
    int i;
...
void PhaseSwitchGfx_BreakIfNoUnits(struct Proc * proc)
{
    ...
    if (GetPhaseAbleUnitCount(gChapterData.currentPhase))
    { // 24CEC
        return;
    }
    EndProc(proc);
...
void FMU_InitVariables(struct FMUProc * proc)
{
    ...
    { // refresh all players when starting FMU
        struct Unit * unit = GetUnit(i);
        if (unit)
        {
...
int FMU_HandleContinuedMovement(void)
{
    ...
    if (gKeyState.pressedKeys & 0xF0)
    { // if pressed a key this frame, prioritize it
    ...
    if (gMapFog[y][x])
    {                                    // after checking that we can go here
...
void EndSupply_StartMMS(void)
{ // replaces a vanilla function
    if (gActiveUnit)
    {
...
int pFMU_MoveUnit(struct FMUProc * proc, u16 iKeyCur)
{ // Label 1
...
int pFMU_HandleKeyMisc(struct FMUProc * proc, u16 iKeyCur)
{ // Label 2
    int result = no_yield;
    ...
    else if (2 & (iKeyCur >> 0x8))
    { // Press L
        pFMU_PressL(proc);
        proc->yield_move = true;
        result = yield;
    }
    else if (1 & (iKeyCur >> 0x8))
    { // Press R
        pFMU_PressR(proc);
        proc->yield_move = true;
        result = yield;
...
int pFMU_HandleSave(struct FMUProc * proc)
{ // KeyPress Default
...
{ // this uses the generic buffer, so it must be used selectively!
...
int BuildStraightLineRangeFromUnit(struct Unit * unit)
{
    ...
    if (unit->supportBits & (1 << 7))
    { //@ Check if unit's DangerRadius is on
        dangerRadius = true;
    }
...
void UpdateSMSDir_All(void)
{
    ...
    for (int i = 1; i < 0xC0; i++)
    {
        ...
        if (unit->pCharacterData->number != ProtagID_Link &&
            (unit->pCharacterData->number < 0xE0 || unit->pCharacterData->number > 0xEF))
        {
            continue;
        }
