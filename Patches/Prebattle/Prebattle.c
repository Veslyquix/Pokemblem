...
void EnemyWeightBonus(struct BattleUnit * bunitA, struct BattleUnit * bunitB)
{
    if (gBattleStats.config & (BATTLE_CONFIG_REAL | BATTLE_CONFIG_SIMULATE))
    {
        if (!(bunitA->weaponAttributes & IA_MAGIC))
        {
            ...
            if ((item == MegaKick_Link) || (item == DoubleKick_Link) || (item == HiJumpKick_Link) ||
                (item == JumpKick_Link) || (item == LowKick_Link) || (item == RollingKick_Link) ||
                (item == Submission_Link) || (item == GrassKnot_Link))
            {
...
// Enemy Str/Mag is reduced by 20% when attacking.
