...
LocationTable CursorLocationTable[] = {
    { (NUMBER_X * 8) - (0 * 8) - 4, Y_HAND * 8 }, { (NUMBER_X * 8) - (1 * 8) - 4, Y_HAND * 8 },
    { (NUMBER_X * 8) - (2 * 8) - 4, Y_HAND * 8 }, { (NUMBER_X * 8) - (3 * 8) - 4, Y_HAND * 8 },
    { (NUMBER_X * 8) - (4 * 8) - 4, Y_HAND * 8 }, { (NUMBER_X * 8) - (5 * 8) - 4, Y_HAND * 8 },
    { (NUMBER_X * 8) - (6 * 8) - 4, Y_HAND * 8 }, { (NUMBER_X * 8) - (7 * 8) - 4, Y_HAND * 8 },
    { (NUMBER_X * 8) - (8 * 8) - 4, Y_HAND * 8 },
};

const u32 DigitDecimalTable[] = { 1, 10, 100, 1000, 10000, 100000, 1000000, 10000000, 100000000, 1000000000 };
...
void StartNumberEntry(ProcPtr parent)
{
    ...
    if (parent)
    {
        proc = (SeedMenuProc *)Proc_StartBlocking((ProcPtr)&SeedMenuProcCmd, parent);
    }
    else
    {
        proc = (SeedMenuProc *)Proc_Start((ProcPtr)&SeedMenuProcCmd, PROC_TREE_3);
    }
    if (proc)
    {
#ifdef POKEMBLEM_VERSION
...
#else
        while (proc->seed > gEventSlots[3])
        {
            proc->seed = proc->seed / 2;
        } // s3 as max
        while (proc->seed < 0)
        {
            proc->seed = (proc->seed * 2) + 1;
        }
        if (proc->seed < gEventSlots[2])
        {
            proc->seed = gEventSlots[2];
        } // s2 as min
#endif
        proc->digit = 0;
        // ResetText();
        ...
        PutDrawText(
            NULL, gBG0TilemapBuffer + TILEMAP_INDEX(12, 1), TEXT_COLOR_SYSTEM_GREEN, 0,
            (GetStringTextLen(string) + 8) / 8, string);
        string = "Match with a friend for the";
        PutDrawText(
            NULL, gBG0TilemapBuffer + TILEMAP_INDEX(8, 4), TEXT_COLOR_SYSTEM_WHITE, 0,
            (GetStringTextLen(string) + 8) / 8, string);
        string = "same randomizer settings.";
        PutDrawText(
            NULL, gBG0TilemapBuffer + TILEMAP_INDEX(8, 6), TEXT_COLOR_SYSTEM_WHITE, 2,
            (GetStringTextLen(string) + 8) / 8, string);
#endif
        ...
        bg = 0;
        InitMultiline(
            (struct Text *)0, lines, x, y, bg, TEXT_COLOR_SYSTEM_WHITE,
            textID[0]); // these functions do nothing if no text ID
        InitMultiline((struct Text *)3, 3, x, y + 10, bg, TEXT_COLOR_SYSTEM_WHITE, textID[1]);
        PrepareMultiline((struct Text *)0, textID[0]);
        PrepareMultiline((struct Text *)3, textID[1]);
        DrawMultiline((struct Text *)0, lines, x, y, bg);
        lines = CountTextIDLines(textID[1]);
        DrawMultiline((struct Text *)3, lines, x, y + 10, bg);
        if (gEventSlots[4])
        {
...
void InitMultiline(struct Text th[], int lines, int x, int y, int bg, int color, int textID)
{
    ...
    // asm("mov r11, r11");
    TileMap_FillRect(TILEMAP_LOCATED(bg_table[bg], x, y), max_width + x, height + y, 0);
...
void DrawMultiline(struct Text th[], int lines, int x, int y, int bg)
{
    ...
    {
        PutText(&th[i], TILEMAP_LOCATED(bg_table[bg], x, (2 * i) + y));
    }
...
static void SeedMenuLoop(SeedMenuProc * proc)
{
    ...
    DisplayVertUiHand(
        CursorLocationTable[proc->digit].x, CursorLocationTable[proc->digit].y); // 6 is the tile of the downwards hand
