import os, sys, glob, codecs, shutil, re, time
from PIL import Image
import numpy as np
import Png2Font
#		python MakeGlyphInstaller.py FE8U.gba

GLYPH_FILE_RE = re.compile(r'^Font(Item|Text)(.*?)(?:_([0-9A-Fa-f]{1,6}))?$')
INVALID_FILENAME_CHARS = set('<>:"/\\|?*')


def IsSafeGlyphNameChar(char):
	if len(char) != 1:
		return False
...
def ParseGlyphFileName(glyph_file):
	glyph_file_name = os.path.basename(glyph_file)
	name_wo_ext = os.path.splitext(glyph_file_name)[0]
	match = GLYPH_FILE_RE.match(name_wo_ext)

	if match is None:
		raise ValueError("invalid glyph filename: " + glyph_file_name)

	glyph_type = match.group(1)
	glyph_text = match.group(2)
	glyph_hex = match.group(3)

	if glyph_hex is not None:
		glyph_codepoint = int(glyph_hex, 16)
	elif len(glyph_text) == 1:
		glyph_codepoint = ord(glyph_text)
	elif len(glyph_text) == 2 and glyph_text[0] == '_':
		glyph_codepoint = ord(glyph_text[1])
	else:
		raise ValueError("glyph filename needs one character or a _HEX suffix: " + glyph_file_name)

	return (glyph_type, glyph_codepoint, name_wo_ext)


def GetGlyphFilePriority(glyph_file, parsed_glyph):
	(glyph_type, glyph_codepoint, name_wo_ext) = parsed_glyph
	try:
		glyph_char = chr(glyph_codepoint)
		if IsSafeGlyphNameChar(glyph_char):
			expected_name = 'Font' + glyph_type + glyph_char
		else:
			expected_name = 'Font' + glyph_type + '_' + format(glyph_codepoint, 'X')
	...
	if re.search(r'_[0-9A-Fa-f]{1,6}$', name_wo_ext):
		return 1
...
def ResetInstallerDir(installer_dir):
	for i in range(5):
	...
	raise OSError("could not reset installer directory: " + installer_dir)


def AppendFontInstaller(glyph_file, installer_dir):


	(glyth_type, glyth_name_uni, name_wo_ext) = ParseGlyphFileName(glyph_file)
...
def main():
	...
	ResetInstallerDir(installer_dir)
	...
	for file in glob.glob(r'FontData/*.png'):
		try:
			parsed_glyph = ParseGlyphFileName(file)
		except ValueError as exc:
			print("! skip: " + str(exc))
			continue

		key = (parsed_glyph[0], parsed_glyph[1])
		old_entry = glyph_entries.get(key)
		new_priority = GetGlyphFilePriority(file, parsed_glyph)

		if old_entry is None or new_priority > old_entry[2]:
			glyph_entries[key] = (file, parsed_glyph, new_priority)
		else:
			print("! duplicate: " + os.path.basename(file))

	glyph_files = [
		entry[0] for entry in sorted(
			glyph_entries.values(),
	...
	for file in glyph_files:
		...
		# make ".fefont" file
		(font_file, file_ext) = os.path.splitext(file)
		font_file = font_file + ".fefont"
		if not os.path.exists(font_file) or os.path.getmtime(font_file) < os.path.getmtime(file):
			Png2Font.Png2Font(file, quiet=True)
